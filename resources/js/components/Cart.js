...
class Cart extends Component {
    ...
    getTotal(cart) {
        const total = cart.map(c => c.pivot.quantity * c.price_with_vat);
        const totalVat = cart.map(c => c.pivot.quantity * c.vat);
        this.state.totalVat = sum(totalVat).toFixed(2);
    ...
    handleClickSubmit() {
        ...
        Swal.fire({
            title: 'Received Amount',
            html: `
                <p>over All Vat:  ${this.state.totalVat}</p>
                <p>Paied Amount: ${window.APP.currency_symbol} ${this.getTotal(this.state.cart)}</p>
            `,
            showCancelButton: true,
            confirmButtonText: 'Send',
            showLoaderOnConfirm: true,
